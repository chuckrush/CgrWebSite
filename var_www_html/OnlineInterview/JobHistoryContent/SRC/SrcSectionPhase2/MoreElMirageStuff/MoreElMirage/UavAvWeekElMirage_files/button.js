...
var commonLoc="&fb=Y&url="+encodeURIComponent(getClickURL())+"&title="+encodeURIComponent(getClickTitle())+"&random="+Math.random()+"&partnerID="+partnerID+"&expire="+encodeURIComponent(getClickExpire());
...
if(!inpop && clickRan>(1-(1/clickFac))){  
    ...
    _qb+="&7="+partnerID;
    _qb+="&8="+encodeURIComponent(getClickURL());
    _qb+="&10="+encodeURIComponent(getClickTitle());
    _qb+="&19="+_ti;
    _qb+="&21="+clickFac;
    _qb+="&18="+Math.random();
     _b.src=_qb;
...
function ET () {
	window.open('http://www.emailthis.clickability.com/et/emailThis?clickMap=create'+commonLoc+'&summary='+encodeURIComponent(getClickSummary())+'&image='+encodeURIComponent(getClickImage()),'click',popWin);
	return false;
}
...
function getClickTitle() {
	...
	if (self.clickTitle) { 
		...
		clktitle=document.title; 
	}
	var rx;
	for (key in repls){
		rx = new RegExp(key, 'g');
		clktitle = clktitle.replace(rx, repls[key]);
	}
	return clktitle;
}
...
if (!custom && !inpop) {
	...
	initBtn(1,1,1,0,0,0,'cc0000','cc0000');
